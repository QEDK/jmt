...
library JellyfishMerkleTreeVerifier {
    ...
    function verifyProof(bytes32 root, Leaf calldata leaf, Proof calldata proof) external pure returns (bool) {
        ...
        else { // Node absence expects exclusion proof
        ...
        if (proof.leaf.addr == 0x0 && proof.leaf.valueHash == 0x0) {
            ...
            calculatedRoot = sha256(abi.encodePacked(hex'4a4d543a3a4c6561664e6f6465', proof.leaf.addr, proof.leaf.valueHash));
        }
        for (uint256 i = 0; i < proof.siblings.length;) {
            if (getIthBit(leaf.addr, proof.siblings.length - i) == 1) {
                calculatedRoot = sha256(abi.encodePacked(hex'4a4d543a3a496e74726e616c4e6f6465', proof.siblings[i], calculatedRoot));
            } else {
                calculatedRoot = sha256(abi.encodePacked(hex'4a4d543a3a496e74726e616c4e6f6465', calculatedRoot, proof.siblings[i]));
            }
            unchecked {
                ++i;
            }
